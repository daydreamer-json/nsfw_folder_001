*
	...
	; メッセージウィンドウの uipsd
	; メッセージウィンドウのオプション
	[meswinopt storage="textwindow"  transparent=true nameAlign=-1 nameVAlign=0 layer=message0 visible=false nameAbsolute=201 textAbsolute=202]
...
*logo
	...
	[se evalstorage="get_random_br(10)"]
	[wait time=2000]
	;[sysvoice wait]
	[se wait]
	[endskip]
...
*title
	...
	[title_show]
	;[sysvoice name="title"]
	[se evalstorage="get_random_ti(10)"]
...
*title_status
	...
	[if exp='tf.title_status=="status_sub"'][クエスト選択 page=status_sub_title]
	[else                                  ][クエスト選択 page=status_main_title]
	[endif]

*close_status
	[sysse name=profile.back]
	[stopvoice all]
	[msgoff]
	[jump target=*title_restore]
